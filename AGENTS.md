...
Static prototype behavior writes `/home/USER/web/DOMAIN/stats/index.html` and records state in `/etc/hestia-goaccess/domains/USER/DOMAIN.conf`. Static reports are regenerated by Hestia's `webstats` queue or manual `v-update-web-domain-stat USER DOMAIN`. Static mode does not yet provide AWStats-style monthly/yearly archives; visible history depends on the active log and server logrotate policy.
